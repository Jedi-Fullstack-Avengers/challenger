    ...
    calc: data => {
      const suma = str => {
        if (
          ...
          (str.match(/a|e|i|o|u/g) || []).length >= 3
        )
          return 1;
        return 0;
      };
      ...
      let acc = 0;
      for (let i = 0; i < data.length; i++) acc += suma(data[i]);
      return acc;
    }
  },
  ...
  present: {
    for: data => {
      ...
      const getArea = str => {
        ...
        const x1 = str.indexOf("x"),
          x2 = str.indexOf("x", x1 + 1);
        const dim0 = str.substring(0, x1),
          dim1 = str.substring(x1 + 1, x2),
          dim2 = str.substr(x2 + 1);
      ...
      for (; ; i++) {
        result +=
          getArea(data[i]) + (last - i > i ? getArea(data[last - i]) : 0);
        if (last - i <= i + 1) break;
      }
    ...
    find: data => {
      ...
      if (xDiff != 0) {
        ...
        for (let i = 0; i < unknowns.length; i++) {
          const iPath = unknowns[i],
            iCoord = iPath + 1;
          let letter = countMax > 0 ? letterMax : letterMin;

          if (
            letter == "r" &&
            (coords[iCoord - 1].x == lastX || coords[iCoord - 1].letter == "l")
          ) {
            letter = "l";
          } else if (
            letter == "l" &&
            (coords[iCoord - 1].x == 0 || coords[iCoord - 1].letter == "r")
          ) {
            letter = "r";
          }
          ...
          coords[iCoord].x =
            letter == "r" ? coords[iCoord - 1].x + 1 : coords[iCoord - 1].x - 1;
        }
      } else if (yDiff != 0) {
        ...
        for (let i = 0; i < unknowns.length; i++) {
          ...
          if (
            letter == "d" &&
            (coords[iCoord - 1].y == lastY || coords[iCoord - 1].letter == "u")
          ) {
            //console.log("iCoord: ", iCoord, " - coords: ", coords);
            letter = "u";
          } else if (
            letter == "u" &&
            (coords[iCoord - 1].y == 0 || coords[iCoord - 1].letter == "d")
          ) {
            letter = "d";
          }
          ...
          coords[iCoord].letter = letter;
          coords[iCoord].y =
            letter == "d" ? coords[iCoord - 1].y + 1 : coords[iCoord - 1].y - 1;
        }
      }
